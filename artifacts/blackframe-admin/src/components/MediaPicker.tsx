...
export function MediaPicker({ value, onChange }: MediaPickerProps) {
  ...
  const addItem = (item: Omit<MediaItem, "id">) => {
    ...
    const newItem: MediaItem = { ...item, id: Date.now().toString() + Math.random() };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    files.forEach(file => {
      if (file.size > MAX_FILE_SIZE) {
        toast.error(`"${file.name}" dépasse 5 MB — ignoré`);
        return;
      }
      ...
      reader.onload = (ev) => {
        ...
        addItem({ type, src, name: file.name });
      };
  ...
  const handleAddUrl = () => {
    ...
    addItem({ type, src: trimmed, name: trimmed });
    setUrlInput("");
  };
      ...
      {tab === "file" && (
        <div>
          ...
          <button
            ...
            className="w-full h-20 rounded-xl border-2 border-dashed border-white/15 bg-white/3 flex flex-col items-center justify-center gap-1.5 text-muted-foreground hover:border-primary/40 hover:text-primary transition-colors active:scale-[0.98]"
            data-testid="media-upload-button"
          >
            <Plus className="w-6 h-6" />
            <span className="text-xs font-medium">Choisir des fichiers (max 5 MB)</span>
          </button>
        </div>
      )}
      ...
      {value.length > 0 && (
        <div className="flex flex-col gap-2 mt-1">
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
            {value.length} média{value.length > 1 ? "s" : ""} ajouté{value.length > 1 ? "s" : ""}
          </p>
          <div className="flex flex-col gap-2">
            {value.map((item, i) => (
              <div
                key={item.id}
                className="flex items-center gap-3 bg-white/5 border border-white/8 rounded-xl p-2 overflow-hidden"
